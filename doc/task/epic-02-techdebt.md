...
**Trạng thái thật**: **6/12 vùng feature** (`cpu`, `gpu`, `ram`, `hardware`, `temp`, `storage` trong `feat/infor`, cộng Applications ở dạng chưa wire) dùng pattern mới (`Frm → VM → Observable*Data(Interactor) → DataProvider`) end-to-end — **quá nửa**. Còn lại `sensor`, `camera`, `screen`, `android`, `drm`, `media` vẫn 100% pattern cũ (ViewModel gọi thẳng Android API/RxJava). `processes` đã xoá hẳn, xem T2.10b.

**Rủi ro để dang dở lâu**: chi phí duplicate tăng dần (case Applications ở B03), 2 mô hình threading (Rx + Flow) tăng rủi ro sửa nhầm — vẫn còn 6 vùng dùng RxJava/API trực tiếp (`sensor`, `camera`, `screen`, `android`, `drm`, `media`).
