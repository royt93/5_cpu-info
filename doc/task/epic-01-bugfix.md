...
| B31 | `FragmentResultListener` xuyên Activity không có tác dụng thật: `ActHost` lắng nghe `KEY_VIP_CHANGED` trên `supportFragmentManager` của chính nó, nhưng `FVipManagement` nằm trong `ActVip` (Activity khác) — event không bao giờ tới `ActHost`. UI hiện "vô tình" cập nhật đúng chỉ vì `ActHost.onResume()` chạy lại khi đóng `ActVip`, không phải do listener hoạt động — fragile, dễ vỡ nếu đổi flow navigation. Fix: dùng `registerForActivityResult` hoặc SharedFlow/DataStore singleton | `feat/ActHost.kt:103-110`, `feat/vip/FVipManagement.kt:331-334` | Gemini CLI | M |
