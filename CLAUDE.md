...
- Provider hiện tại: **AdMob** (active, `BuildConfig.IS_ENABLE_ADMOB = true` cho cả debug + release) thông qua SDK wrapper `com.github.royt93:AdmobApplovinWrapper:1.8.5` (cài qua JitPack — repo đã được thêm vào `allprojects.repositories`). Flip `IS_ENABLE_ADMOB = false` trong `app/build.gradle.kts` để quay lại AppLovin MAX (fallback). Cả 5 ad-unit ID production (Banner/Interstitial/AppOpen/Rewarded/App ID) đều là ID thật (`ca-app-pub-3004713799155145/...`), đọc từ `ads.properties` (myKeyStore) — xác nhận lại 2026-09-09, dòng ghi "Rewarded vẫn test ID" trước đây là comment cũ sai, đã sửa.
