...
class SystemInfoExporter @Inject constructor(
        ...
        scope.launch {
            val body = withContext(dispatchersProvider.io) {
                when (format) {
                    ...
                    Format.HTML -> buildSystemInfoHtml()
    ...
    private fun buildSystemInfoHtml(): String = buildString {
        appendLine("<!DOCTYPE html><html><head><meta charset=\"utf-8\">")
        appendLine("<title>System Information - ${Build.MODEL.htmlEscape()}</title>")
        appendLine(
            "<style>body{font-family:sans-serif}" +
                "h2{border-bottom:1px solid #ccc;padding-bottom:4px}" +
                "table{border-collapse:collapse;margin-bottom:16px}" +
                "td{padding:2px 12px 2px 0}</style>",
        )
        appendLine("</head><body>")
        appendLine("<h1>System Information</h1>")

        htmlSection("Device") {
            htmlRow("Model", Build.MODEL)
            htmlRow("Brand", Build.BRAND)
            htmlRow("Manufacturer", Build.MANUFACTURER)
            htmlRow("Android", "${Build.VERSION.RELEASE} (API ${Build.VERSION.SDK_INT})")
        }

        htmlSection("CPU") {
            htmlRow("Processor", dataNativeProviderCpu.getCpuName())
            htmlRow("ABI", dataProviderCpu.getAbi())
            val cpuCount = dataProviderCpu.getNumberOfCores()
            htmlRow("Cores", cpuCount.toString())
            for (i in 0 until cpuCount) {
                val freq = dataProviderCpu.getCurrentFreq(i)
                if (freq > 0) htmlRow("Core $i", "${freq}MHz")
            }
        }
        ...
        htmlSection("RAM") {
            val totalRam = dataProviderRam.getTotalBytes() / GIGA
            val availRam = dataProviderRam.getAvailableBytes() / GIGA
            htmlRow("Total", "${formatTwoDecimals(totalRam)}GB")
            htmlRow("Available", "${formatTwoDecimals(availRam)}GB")
            htmlRow("Usage", "${100 - dataProviderRam.getAvailablePercentage()}%")
        }

        htmlSection("Storage") {
            val internalPath = Environment.getDataDirectory()
            htmlRow("Internal Total", "${formatTwoDecimals(internalPath.totalSpace / GIGA)}GB")
            htmlRow("Internal Free", "${formatTwoDecimals(internalPath.usableSpace / GIGA)}GB")
        }

        htmlSection("GPU") {
            htmlRow("GLES Version", dataProviderGpu.getGlEsVersion())
            htmlRow("Vulkan Version", dataProviderGpu.getVulkanVersion())
        }

        val display = displayManager.getDisplay(Display.DEFAULT_DISPLAY)
        if (display != null) {
            htmlSection("Display") {
                htmlRow("Refresh rate", "${formatTwoDecimals(display.refreshRate.toDouble())} Hz")
                htmlRow("Supported modes", display.supportedModes.size.toString())
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                    val hdr = display.hdrCapabilities?.supportedHdrTypes ?: intArrayOf()
                    htmlRow("HDR types", if (hdr.isEmpty()) "None" else hdr.size.toString())
                }
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                    htmlRow("Wide color gamut", display.isWideColorGamut.yesNo())
                }
            }
        }

        htmlSection("Sensors") {
            htmlRow("Total", sensorManager.getSensorList(Sensor.TYPE_ALL).size.toString())
        }

        htmlSection("Cameras") {
            htmlRow("Total", cameraIds().size.toString())
        }

        htmlSection("Media Codecs") {
            val codecs = MediaCodecList(MediaCodecList.REGULAR_CODECS).codecInfos
            htmlRow("Total", codecs.size.toString())
            htmlRow("Decoders", codecs.count { !it.isEncoder }.toString())
            htmlRow("Encoders", codecs.count { it.isEncoder }.toString())
        }

        htmlSection("DRM") {
            htmlRow("Widevine", widevineSecurityLevel() ?: "Not supported")
        }

        appendLine("<p><em>Generated by CPU Info App</em></p>")
        appendLine("</body></html>")
    }

    private fun StringBuilder.htmlSection(title: String, rows: StringBuilder.() -> Unit) {
        appendLine("<h2>${title.htmlEscape()}</h2><table>")
        rows()
        appendLine("</table>")
    }

    private fun StringBuilder.htmlRow(label: String, value: String) {
        appendLine("<tr><td><strong>${label.htmlEscape()}</strong></td><td>${value.htmlEscape()}</td></tr>")
    }

    internal fun String?.htmlEscape(): String =
        orEmpty().replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
